...
class Brownian(object):
    ...
    def catch_path_is_outside(self,old_pos):
        ...
        if not self.settings.boundary.contains(new_pos):
            ...
            raise Exception("Path is outside of boundary and could not change course correcytly. Tr again")
    ...
    def generate_new_pos(self, old_pos: geometry.Point):
        ...
        if change_grazing_zone < self.settings.migration_factor:
            ...
            while (not self.settings.boundary.contains(new_pos)) or (self.line_intersects(old_pos, new_pos)):
                new_pos = self.find_random_point_in_polygon()
